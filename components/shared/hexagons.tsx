import { Avatar, Box, Container, Link, Tooltip, useTheme } from "@mui/material";
...
const Hexagons = ({ hexagonSize }: Props) => {
    ...
    return (
        ...
        >
            {constants.links.profiles.map(
                ({ title, url, imagePath }, index) => {
                    return (
                        <Fragment key={`profile item number: ${index}`}>
                            <Tooltip title={footer.profilesTitles[title]} arrow>
                                <Box
                                    sx={{
                                        position: "absolute",
                                        top: dimensions(
